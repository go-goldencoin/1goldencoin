...
GLDC_Reserve="NM6F3w2Dmbccx44xUQ3e3d63MWtk1eMWDk"
...
GLDC_Var_Bal=$(get_neng_balance)
CHTA_Var_Bal=$(get_chta_balance)

function Tx_2_GLDC_Reserve(){
        $VAR1 sendtoaddress $GLDC_Reserve $GLDC_Var_Bal
}
...
## Send to reserve
if [ $GLDC_Var_Bal == $Min_Bal ]
then
        echo "Local GLDC Balance = " $GLDC_Var_Bal
        echo "Value is Zero, therefore nothing to do."
else
        echo "GLDC Balance is not Zero = " $GLDC_Var_Bal
        echo "Sending balance to reserve wallet."
        Tx_2_GLDC_Reserve
        echo "Done"
        echo "Local GLDC Wallet New Balance: "
        get_neng_balance
fi
