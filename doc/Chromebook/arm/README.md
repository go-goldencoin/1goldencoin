...
This folder describe how to run a full node of Goldencoin (GLDC) and to CPU mine GLDC in arm based Chromebook as Linux App.  Chrome OS on arm64 or armhf platform was tested. Any Chrome OS version v69 or later on 64 bits arm64 or 32 bits armhf  with linux either in linux beta (Crostini) or Android UserLand app should all work. 
...
Go to Chrome OS setting, turn on Linux (Beta).  If your chromebook does not have this option, it is not supported here for GLDC CPU mining. However, your arm CPU based chromebook may still work on Android UserLand app method as alternative. 
...
```
 ...
 Then restart the QT GUI wallet, GLDC should start to sync. Note fully synced wallet to latest block is required in order to CPU mine GLDC with Cheetah_Cpuminer
