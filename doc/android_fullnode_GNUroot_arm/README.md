...
This folder describes how to set up full node of Goldencoin (GLDC) on 32 bits android phone on arm CPUs. 
...
https://github.com/ShorelineCrypto/GoldenCoin/releases/download/GLDC_2020_Q3_report/GLDC_2020_Q3_report.pdf 
...
### Step 9: install some tools, prepare GLDC required library

 screen is useful tools to run in background.  busybox top or uptime is better tool for 
than the default ones.  "busybox top" can see GLDC mining command and android processes.  
...
## From Desktop Linux in Virtualbox
### Step 10: Compile GLDC wallet in QEMU
  The armel/armhf GLDC binary from our download is unlikely to work well if your phone hardware is different. 
  Explained in pdf report, you should compile Berkley DB libary above successfully, but likely to fail on GLDC core compilation.  Extract and copy DB library files into QEMU and compile GLDC core wallet following cloud boot from below guide:
 ...
 After successfully compiling the GLDC wallet files, copy the files into the android phone for use. 
...
```
   ...
   GLDC full node can run in one screen session, the cheetah runs in another screen session. 
