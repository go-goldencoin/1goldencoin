...
This folder describe how to CPU mine Goldencoin (GLDC) in android phone on 64 bits (arm64) or 32 bits (armhf) through UserLand app inside a linux terminal.  UserLand app is an open-source app that can run linux inside your android phone. 
...
https://github.com/ShorelineCrypto/NewEnglandCoin/releases/download/GLDC_2020_Q3_report/GLDC_2020_Q3_report.pdf 
...
## Confusion on android vs linux on GLDC v1.4.x/v1.5.x release
...
Not trying to create a flaming war.  Understanding the relationship between android vs linux, then you will clearly know what we did on GLDC android CPU mining through UserLand app inside a linux terminal at almost native OS speed.
...
Arm hardware may have hardware compatibility issues. This situation is worse on 32 bits arm platform. You can suspect a hardware issue if you run into error like "error while loading shared libraries: libboost_system.so.1.58.0". Or GLDC wallet may run, but crash within 1 or 2 hours.

Here we provide a Ubuntu 18.04 GLDC wallet method where you can compile Boost library from source. With self-compiled boost library from source together with GLDC binary wallet file, the hardware compatibility issue should be fixed.


## Linux Terminal (SSH) or Desktop (VNC or XSDL)? 

For GLDC android mining purpose, we recommend using Linux terminal only through UserLand SSH login mode.  Our experience on debian or ubuntu has been fabulous and smooth with 
...
Android is optimized for mobile GUI on top of linux kernel. GNU or regular linux LXDE or XFCE desktop is mainly for desktop/laptop environment. Therefore we see limited value using LXDE/XFCE in android phones any way. If you want to learn or have fun experience for linux, we would recommend to install a copy of Linux OS inside "VirtualBox" in windows 10. Oracle VirtualBox is free virtual machine software that can install and run a second operating system in windows 10 without need of dual boot.  With virutalBox setup for any one of 8 distros that we support for GLDC wallet, you can mine GLDC with cheetah_cpuminer easily in linux inside virtualbox of windows 10. 
...
Some members reported that putty does not work either because new android versions have security feature to block UserLand app port or routers block those ports.  Member @Sefia at discord provided this word documents "GLDC_Android_arm64_Mining_Guide_without_Putty.txt" at current folder on how to do android mining without using putty. 
...
## optional - Load GUI QT wallet into windows/mac/linux Desktop Remotely
 ...
 Running linux terminal-only does not mean that you can not load GLDC QT wallet with GUI remotely. If your home wifi speed is decent, you could try to load GLDC QT GUI wallet in windows 10, or MacOS or Linux Desktop PCs. 
...
```
 ...
 hlu@localhost:~/goldencoin_v1.7.0.2_u16_arm64$ ./goldencoin-qt &
```
Above in mac terminal will pop up android phone GLDC QT GUI wallet in macOS desktop. 
...
```
 ...
 hlu@localhost:~/goldencoin_v1.7.0.2_u16_arm64$ ./goldencoin-qt &
```
Above in linux desktop terminal will pop android phone GLDC QT wallet in linux desktop remotely.
